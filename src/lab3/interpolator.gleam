...
import lab3/types.{type Config, type Message, type State, AddPoint, Linear, Newton, Shutdown, State}
import lab3/output

pub fn start(config: Config) -> Result(process.Subject(Message), actor.StartError) {
  case actor.new(State(points: [], config: config, last_output_x: None))
    ...
    AddPoint(point) -> {
      ...
      case list.length(new_points) >= min_points {
        True -> {
          ...
          let new_last_x = output.interpolate_and_output(window, state.config, state.last_output_x)
        ...
        True, Ok(last_point) -> {
          case state.last_output_x {
            Some(last_x) -> {
              ...
              let _ = output.output_range(window, state.config, last_x +. state.config.step, last_point.x)
              Nil
            }
            None -> Nil
          }
